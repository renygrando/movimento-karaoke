...
export function KaraokeProvider({ children }: { children: ReactNode }) {
  ...
  const [discoveredSongs, setDiscoveredSongs] = useLocalStorage<
    Record<string, Song>
  >("karaoke-discovered-songs", {});

  const getSongById = useCallback(
    (songId: string): Song | undefined => {
      return (
        (discoveredSongs || {})[songId] ||
        songDatabase.find((s: Song) => s.id === songId)
      );
    },
    [discoveredSongs]
  );

  const addDiscoveredSong = useCallback(
    (song: Song) => {
